...
function displayProducts() {
    ...
    // Filter products to only include those in the "men clothes" category
    const menClothesProducts = dataPro.filter(product => product.category.toLowerCase() === 'men clothes');

    const container = document.getElementById('products-container');
    container.innerHTML = '';

    menClothesProducts.forEach(product => {
        const productHTML = `
    ...
    document.querySelectorAll('.product-link, .product-thumb').forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            const selectedProduct = menClothesProducts.find(product => product.title === productId);
